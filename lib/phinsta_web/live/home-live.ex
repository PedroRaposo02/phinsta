defmodule PhinstaWeb.HomeLive do
  ...
  def render(assigns) do
    ~H"""
    <h1 class="text-2xl py-4">Phinsta</h1>
    ...
    <.modal id="new-post-modal">
      <.simple_form id="post-form" for={@form} phx-change="validate" phx-submit="save-post">
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Phinsta.PubSub, "posts")
  ...
  def handle_event("save-post", %{"post" => post_params}, socket) do
    ...
    |> case do
      {:ok, post} ->
        ...
        Phoenix.PubSub.broadcast(Phinsta.PubSub, "posts", {:new, Map.put(post, :user, user)})
  ...
  defp consume_files(socket) do
    consume_uploaded_entries(socket, :image, fn %{path: path}, _entry ->
      dest = Path.join([:code.priv_dir(:phinsta), "static", "uploads", Path.basename(path)])
      File.cp!(path, dest)
